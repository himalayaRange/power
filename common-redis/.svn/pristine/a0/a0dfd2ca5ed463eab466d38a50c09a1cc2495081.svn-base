package com.bly.common.redis.util;
...
import com.bly.common.utils.ConfigurationUtil;
...
public class RedisUtil {
	...
	private RedisUtil(){
		if(pool == null){
			Configuration configuration = ConfigurationUtil.getPropertiesConfiguration("redis.properties");
			String ip = configuration.getString("redis.host");
			String password =configuration.getString("redis.password");
			int db = configuration.getInt("redis.db");
			int port = configuration.getInt("redis.port");
			int maxIdle = configuration.getInt("redis.maxIdle");
			int minIdle = configuration.getInt("redis.minIdle");
			int maxTotal = configuration.getInt("redis.maxTotal");
			long maxWaitMillis = configuration.getLong("redis.maxWaitMillis");
			...
			pool = new JedisPool(poolConfig, ip,port,timeout,password,db);
		}
	}
	...
	/**
	 * <p> 向redis中存入key和value，并释然连接资源  </p>
	 * <p> key存在的话则覆盖 </p>
	 * @param key
	 * @param value
	 * @return 成功返回OK，失败返回0
     ...
     * @param keys String数组也可以是一个
     * @return 成功返回value集合，失败返回null，异常返回空
     */
     ...
     * <p>example:</p> 
     * <p>obj.mset(new String[]{"key1","value1","key2","value2"})</p> 
     * @param keysvalues 
	...
	public static RedisUtil getRedisConnection(){
		return ru;
	}
